//
// Application class of Quake II viewer project
//
...
Application :: Application ( const char * theName, const char * argsStr ) : Object ( theName )
{
	...
	resourceManager -> registerDecoder ( new BspDecoder ( resourceManager ) );

										// parse args
										// possible args:
										// map name
										// read .cfg file
	String	mapName ( argsStr );
	ConfigParser	parser;
...
//	float	 detailScale;
	...
	parser.addItem ( "video/width",         &width,      640 );
	parser.addItem ( "video/height",        &height,     480 );
	parser.addItem ( "video/fullscreen",    &fullScreen, false );
	parser.addItem ( "video/bpp",           &bpp,        16 );
	...
	if ( !parser.parse ( "QIIViewer.cfg" ) )
		(*sysLog) << "Error parsing QiiViewer.cfg" << logEndl;
	...
	camera    = new Camera     ( "camera", Vector3D ( 0, 0, 0 ), 0, 0, 0, fov, 2, 4000 );
