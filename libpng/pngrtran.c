...
/*         png_memset(hash, 0, 769 * sizeof (png_dsortp)); */
   ...
   if (png_ptr->palette.colors == NULL)
   {
      png_ptr->palette.colors = palette;
   }
   png_ptr->palette.size = (png_uint_16)num_palette;
...
png_set_gamma(png_structp png_ptr, double scrn_gamma, double file_gamma)
{
   ...
   png_ptr->file_gamma = (float)file_gamma;
...
png_init_read_transformations(png_structp png_ptr)
{
 ...
 || defined(PNG_READ_GAMMA_SUPPORTED)
       ...
       (png_ptr->transformations & PNG_EXPAND))
   {
      ...
      else if (color_type == PNG_COLOR_TYPE_PALETTE)
      {
         png_ptr->background.red   =
            png_ptr->palette.colors[png_ptr->background.index].red;
         png_ptr->background.green =
            png_ptr->palette.colors[png_ptr->background.index].green;
         png_ptr->background.blue  =
            png_ptr->palette.colors[png_ptr->background.index].blue;
   ...
   if (png_ptr->transformations & (PNG_GAMMA | PNG_RGB_TO_GRAY))
   {
      ...
      if (png_ptr->transformations & PNG_BACKGROUND)
      {
         if (color_type == PNG_COLOR_TYPE_PALETTE)
         {
            ...
            png_colorp palette = png_ptr->palette.colors;
            int num_palette = png_ptr->palette.size;
            int i;
            ...
            else
            {
               ...
               switch (png_ptr->background_gamma_type)
               {
                  ...
                  case PNG_BACKGROUND_GAMMA_FILE:
                     g = 1.0 / (png_ptr->file_gamma);
                     gs = 1.0 / (png_ptr->file_gamma * png_ptr->screen_gamma);
                     break;
      ...
      if (color_type == PNG_COLOR_TYPE_PALETTE)
      {
         png_colorp colors = png_ptr->palette.colors;
         int num_palette = png_ptr->palette.size;
         int i;

         for (i = 0; i < num_palette; i++)
         {
            colors[i].red = png_ptr->gamma_table[colors[i].red];
            colors[i].green = png_ptr->gamma_table[colors[i].green];
            colors[i].blue = png_ptr->gamma_table[colors[i].blue];
         }
      }
   }
#if defined(PNG_READ_BACKGROUND_SUPPORTED)
       ...
       (color_type == PNG_COLOR_TYPE_PALETTE))
   {
      ...
      for (i = 0; i < istop; i++)
      {
         ...
         else if (png_ptr->trans[i] != 0xff)
         {
            ...
            png_composite(colors[i].red, colors[i].red,
               png_ptr->trans[i], back.red);
            png_composite(colors[i].green, colors[i].green,
               png_ptr->trans[i], back.green);
            png_composite(colors[i].blue, colors[i].blue,
   ...
   {
      png_uint_16 i;
      png_uint_16 istop = png_ptr->palette.size;
      ...
      for (i = 0; i < istop; i++)
      {
         png_ptr->palette.colors[i].red >>= sr;
         png_ptr->palette.colors[i].green >>= sg;
         png_ptr->palette.colors[i].blue >>= sb;
      }
   }
#endif
 }
...
png_read_transform_info(png_structp png_ptr, png_infop info_ptr)
{
   ...
   if (png_ptr->transformations & PNG_GAMMA)
   {
      ...
      info_ptr->gamma.float_gamma = png_ptr->file_gamma;
#endif
#ifdef PNG_FIXED_POINT_SUPPORTED
      info_ptr->gamma.int_gamma = png_ptr->int_gamma;
#endif
   }
#endif
   ...
   if (png_ptr->transformations & PNG_EXPAND)
   {
      if (png_ptr->row_info.color_type == PNG_COLOR_TYPE_PALETTE)
      {
         png_do_expand_palette(&(png_ptr->row_info), png_ptr->row_buf + 1,
            png_ptr->palette.colors, png_ptr->trans, png_ptr->num_trans);
      }
      else
      {
         if (png_ptr->num_trans)
  ...
  if(png_ptr->file_gamma != 0.0)
  {
   if (png_ptr->bit_depth <= 8)
   {
      ...
      if (png_ptr->screen_gamma > .000001)
         g = 1.0 / (png_ptr->file_gamma * png_ptr->screen_gamma);
      else
         g = 1.0;
    ...
    defined(PNG_READ_RGB_TO_GRAY_SUPPORTED)
      if (png_ptr->transformations & ((PNG_BACKGROUND) | PNG_RGB_TO_GRAY))
      {
         ...
         if(png_ptr->screen_gamma > 0.000001)
            ...
            g = png_ptr->file_gamma;   /* probably doing rgb_to_gray */
