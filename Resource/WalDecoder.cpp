...
struct WalHeader						// header of .val file
{
...
MetaClass	WalDecoder :: classInstance ( "WalDecoder", &ResourceDecoder :: classInstance );

bool	WalDecoder :: checkExtension ( const String& theName )
{
	return String :: getExtension ( theName ).toLower () == "wal";
}

Object * WalDecoder :: decode ( Data * data )
{
	if ( data -> getLength () < sizeof ( WalHeader ) )
		return NULL;

	WalHeader * hdr    = (WalHeader *) data -> getPtr ();
	int			width  = hdr -> width;
	int			height = hdr -> height;

							// now do some checks whether this is a true .wal file
	if ( width < 1 || width > 256 || height < 1 || height > 256 )
		return NULL;

	if ( data -> getLength () < (int)sizeof ( WalHeader ) + width * height )
		return NULL;
	...
	data -> seekCur ( hdr -> offsets [0] );	// get offset to largest texture
	...
	for ( int y = 0; y < height; y++ )
	{
		for ( int x = 0; x < width; x++ )
		{
			...
			pixelBuf [x] = rgbToInt ( walPalette [c].blue, walPalette [c].green, walPalette [c].red );
		}
